...
Provides comprehensive sentiment analysis insights with aggregated statistics,
word impact visualization, and keyword tracking across all analyzed texts.

Features:
- Quick overview metrics (total analyses, sentiment counts, keyword tracking)
- Interactive sentiment distribution pie chart
- Word impact visualization with horizontal bar charts
- Detailed keyword tables (positive/negative words with frequency & impact)
- DSA insights explaining underlying algorithms
- Export functionality for comprehensive reports

DSA Complexity:
- render(): O(n * m) where n = history length, m = avg words per text
